...
class OpenCodeGoProvider implements AiProvider
{
    ...
    public function chat(array $messages): string
    {
        ...
        if ($response->failed()) {
            \Illuminate\Support\Facades\Log::error('OpenCodeGo chat failed', [
    ...
    public function stream(array $messages): Generator
    {
        ...
        if ($response->failed()) {
            \Illuminate\Support\Facades\Log::error('OpenCodeGo stream failed', [
                'status' => $response->status(),
                'body' => $response->body(),
            ]);
        ...
        try {
            ...
            \Illuminate\Support\Facades\Log::error('OpenCodeGo stream detach failed', [
                'error' => $e->getMessage(),
            ]);
        }
